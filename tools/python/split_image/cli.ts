...
async function convertAllImages() {
    ...
    var gifDir = argv._[0]
    var pngs = getPngs(gifDir)
    var cdir = path.resolve(gifDir+"", "cfiles")

    if(!pngs){
        console.error("没有找到png文件");
        process.exit(-123);
    }
    
    for(const imagePath of pngs) {
        console.log("Beginning conversion of " + imagePath);
        const imageName = argv.i ? argv.i : getFileName(path.basename(imagePath as string));
        const cFileString = await convert(imagePath, { cf: ImageMode.CF_INDEXED_4_BIT, outputFormat: outputMode, binaryFormat: ImageMode[BINARY_FORMAT_PREFIX + binaryFormat], swapEndian: argv.s, imageName: imageName, dith:argv.d });
        const outputPath: string = (path.resolve(cdir, (argv.o ? argv.o : getCFilePath(imageName, outputMode))+"")) as any;
